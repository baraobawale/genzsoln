...
public class UC17_18_19_20_21_TradingKauf {
		...
		try {
			...
			if (commonActions.getScenarioName().equals("KaufOrder_Anlegen_Anleihe")) {
				...
				System.out.println("capturedNominalStueck_Betrag: " + CapturedNominalBetrag);
					...
					|| commonActions.getScenarioName().equals("KaufOrder_Anlegen_Fonds2")) {
			...
			if (commonActions.getScenarioName().equals("KaufOrder_Anlegen_Anleihe")) {
				...
				if (Nominal_Betrag.equals(CapturedNominalBetrag)) {
					...
					commonActions.logFailStatus(
							"Fail | Valid NominalStueck_Betrag value display failed - " + Nominal_Betrag);
				}
			} else if (commonActions.getScenarioName().equals("KaufOrder_Anlegen_Aktie")
					|| commonActions.getScenarioName().equals("KaufOrder_Anlegen_Fonds1")
					|| commonActions.getScenarioName().equals("KaufOrder_Anlegen_Fonds2")) {
				...
				if (Stueck_Betrag.equals(CapturedStueck_Betrag)) {
					...
					System.out.println("Valid NominalStueck_Betrag value display failed");
					commonActions
							.logFailStatus("Fail | Valid NominalStueck_Betrag value display failed - " + Stueck_Betrag);
				}
			} else {
	...
	public void Then_Verify_Message_details_on_OrderAendern() {
		try {
			...
			e.printStackTrace();
			// commonActions.logAssert_Fail("Verify Message,details on OrderAendern
			// Failed");
		}
	}
	...
	public void Then_Verify_Message_on_OrderLoeschen() throws FileNotFoundException, IOException, ParseException {
		try {
			...
			e.printStackTrace();
			// commonActions.logAssert_Fail("Verify Message,details on OrderLoeschen
			// Failed");
		}
	}

	@When("User clicks on {string} of Created_WKN,Status in CreatedWKN_OrderUebersicht with Status_OrderUebersicht")
	public void user_clicks_on_of_Created_WKN_Status_in_CreatedWKN_OrderUebersicht_with_Status_OrderUebersicht(
			String locatorKey) throws FileNotFoundException, IOException, ParseException {
		String Status = commonActions.getValueFromJson("Status");
		System.out.println("Status " + Status);
		try {
			if (commonActions.getScenarioName().equals("KaufOrder_Anendern_Aktie")
					&& Status.equals(commonActions.getText("Status_KaufOrder_Anendern_Aktie"))) {
				locatorKey = "KaufOrder_Anendern";
				commonActions.click(locatorKey);
				System.out.println("Anendern SUCCESSFUL ");
			}
			if (commonActions.getScenarioName().equals("KaufOrder_Loeschen_Aktie")
					&& Status.equals(commonActions.getText("Status_OrderUebersicht_Loeschen_Aktie"))) {
				locatorKey = "DeleteAktie";
				commonActions.click(locatorKey);
				if (commonActions.isElementPresent("Order_is_executed_and_cannotbedeleted")) {
					commonActions.logAssert_Fail("\r\n"
							+ "The order can not be canceled because the status of the order does not permit this.");
					System.out.println(
							"The order can not be canceled because the status of the order does not permit this.");
				}
			}

			if (commonActions.getScenarioName().equals("KaufOrder_Loeschen_Fond")
					&& Status.equals(commonActions.getText("Status_OrderUebersicht_Loeschen_Fond"))) {
	...
	public void User_click_on_WeiterTANEingabe_OrderAendern() throws InterruptedException {
		try {
			commonActions.click("WeiterTANEingabe_OrderAendern");
			if (commonActions.isElementPresent("Status_of_order_not_allow_this")) {
				commonActions.logAssert_Fail(
						"\r\n" + "The order can not be changed because the status of the order does not allow this.");
				System.out.println("The order can not be changed because the status of the order does not allow this");
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			commonActions
					.logAssert_Fail("The order can not be changed because the status of the order does not allow this");
		}
