...
public class UC16_Dauerlastschrift {
	...
	public void verify_under_LaufendenZahlungenVorlagen_UmsaetzeZahlungsverkehr(String string, String string2) {
		...
		if (VerifyStartdatumDauerlastschrift.equals(CapturedStartdatumDauerlastschrift)) {
			...
			commonActions
					.logPassStatus("Pass | Valid Startdatum value displayed - " + VerifyStartdatumDauerlastschrift);
		} else {
			commonActions.logFailStatus(
					"Fail | Valid Startdatum value display failed - " + VerifyStartdatumDauerlastschrift);
		}
	...
	public void verify_captured_details_Message_from_DauerlastschriftAnlengen() {
		try {
			...
			if (MessageJson.equals(MessageDauerlastschrift)) {
				...
				commonActions
						.logPassStatus("Pass | Valid MessageDauerlastschrift displayed " + MessageDauerlastschrift);
			} else {
				System.out.println("Message Fail ");
				commonActions.logFailStatus(
						"Fail | Valid MessageDauerlastschrift display failed -" + MessageDauerlastschrift);
			}
		} catch (Exception e) {
			e.printStackTrace();
